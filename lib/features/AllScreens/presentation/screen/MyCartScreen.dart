...
class MyCartScreen extends StatelessWidget {
  ...
  Future<void> _placeOrder(BuildContext context,
      CalciumMineralProductProvider cartProvider, ApiService apiService) async {
    final selected = cartProvider.selectedProducts;
    final quantities = cartProvider.quantities;

    if (selected.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text("Please add the products")));
      return;
    }
    ...
    //     content: const Text('How would you like to pay for your order?'),
    //     actions: [
    //       TextButton(
    //         onPressed: () => Navigator.pop(context, false), // false for COD
    //         child: const Text('Cash on Delivery'),
    //       ),
    //       TextButton(
    //         onPressed: () => Navigator.pop(context, true), // true for Online
    ...
    if (paymentMethod) {
      ...
      try {
        ...
        bool startPaymentResult = await EasebuzzPaymentHandler.startPayment(
          ...
          selected,
          quantities,
          address, // Pass address to payment handler
        );
...
// Widget build(BuildContext context) {
  ...
  //           hintText: 'Enter your complete address with landmark',
  //           border: OutlineInputBorder(),
  //         ),
  //         maxLines: 4,
  //         validator: (value) {
