...
class _EditPostScreenState extends State<EditPostScreen> {
  ...
  late final _descC       = TextEditingController(text: widget.post.description ?? '');
  ...
  late final _shopNameC   = TextEditingController(text: widget.post.shopName ?? '');
  late final _addressC    = TextEditingController(text: widget.post.address ?? '');
  late final _ghabhanC    = TextEditingController(text: widget.post.ghabhanMonth ?? '');
  ...
  void dispose() {
    for (final c in [
      _nameC,_descC,_ageC,_vetC,_milkC,_weightC,_priceC,
      _useYearC,_shopNameC,_addressC,_ghabhanC
    ]) { c.dispose(); }
    super.dispose();
  }
  ...
  Future<void> _save() async {
    ...
    try {
      ...
      if (widget.post.description != null) req.fields['description'] = _descC.text;
      ...
      if (widget.post.shopName != null) req.fields['shop_name'] = _shopNameC.text;
      if (_isGhabhan != null) req.fields['is_ghabhan'] = _isGhabhan!;
      if (widget.post.ghabhanMonth != null) req.fields['ghabhan_month'] = _ghabhanC.text;
      ...
      body: _loadingUnits
        ...
        child: Column(
          children: [
            ...
            if (widget.post.weight != null && widget.post.weight!.isNotEmpty)
              Row(
                children: [
            ...
            if (widget.post.price != null && widget.post.price!.isNotEmpty)
              _txt(_priceC, 'किंमत', number: true),

            // Only show use year field if post has use year
            if (widget.post.useYear != null && widget.post.useYear!.isNotEmpty)
              _txt(_useYearC, 'वापर वर्षे', number: true),

            // Only show shop name field if post has shop name
            if (widget.post.shopName != null && widget.post.shopName!.isNotEmpty)
            ...
            if (_isGhabhan == 'होय' && widget.post.ghabhanMonth != null && widget.post.ghabhanMonth!.isNotEmpty)
            ...
            SizedBox(
              ...
              child: ElevatedButton(
                ...
                child: _saving
                    ...
                    : const Text('सेव्ह करा',style: TextStyle(color: Colors.white, fontSize: 16)),
...
class UpdatedData {
  ...
  final String? name, description, age, vet, milk, weight, unit,
      price, useYear, shopName, photo, isGhabhan, ghabhanMonth;
